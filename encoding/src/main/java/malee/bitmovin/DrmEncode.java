...
public class DrmEncode
{

	//  Bitmovin Encoding API Key
	private static String ApiKey = "c163641f-65e2-4ada-893f-10f1dd6d2b84";

    // Define Cloud Region (Used for Non Premium Content)
	private static CloudRegion cloudRegion = CloudRegion.AUTO;
    
	// For Premium Content, likely to be on-premise. Commented out for this sample.
	// Defines that Kubernetes will be in use, and Infrastructure ID to be used
	
	//private static CloudRegion cloudRegion = CloudRegion.KUBERNETES;
	//private static String infrastructureId = "<INSERT-KUBERNETES-ID>"
    
    
	// For Non-Premium Content, download directly from the cloud
	private static String HTTP_INPUT_HOST = "<HTTP_HOSTNAME>"; // ex.: storage.googleapis.com/
    private static String HTTP_INPUT_PATH = "<ABSOLUTE_FILE_PATH>"; // ex.: /343291949.mp4
    
    // For Premium Content (encoded on Kubernetes cluster), use a local input
    //private static String LOCAL_INPUT_ROOT = "<LOCAL_FILE_ROOT>"; // ex.: /mnt/storage/
    //private static String LOCAL_INPUT_PATH = "<ABSOLUTE_FILE_PATH>"; // ex.: /343291949.mp4
    
    // We will write all output content to S3 to use as Origin Storage
    private static String S3_OUTPUT_ACCESSKEY = "<S3_ACCESS_KEY>";
    private static String S3_OUTPUT_SECRET_KEY = "<S3_SECRET_KEY>";
    private static String S3_OUTPUT_BUCKET_NAME = "<S3_BUCKET>";
    private static String OUTPUT_BASE_PATH = "" + new Date().getTime();

    // BEGIN DRM Definition
    
    // Define PlayReady License Details (Details for demonstration only)
    private static String PLAYREADY_KID = "	0102030405060708090AAABBCCDDEEFF";
    //private static String PLAYREADY_KEY = "GUf166PQbx+sgBADjyBMvw=="; //Either KEY or KEYSEED is required
    private static String PLAYREADY_KEYSEED = "XVBovsmzhP9gRIZxWfFta3VVRPzVEWmJsazEJ46I"; //Either KEY or KEYSEED is required (please see line 509 if you want to use keySeed)
    private static String PLAYREADY_LAURL = "http://test.playready.microsoft.com/service/rightsmanager.asmx?cfg=(persist:false,sl:150)";

    // Define Widevine License Details (Details for demonstration only)
    private static String WIDEVINE_KID = "AD11873D647E27483F336503161D715B";
    private static String WIDEVINE_KEY = "344A9430A36BB073B1E97A4FF185900E";
    private static String WIDEVINE_PSSH = "EhDTHfVHTbdUu797gh8QdN0QGgZ2dWFsdG8iEkxVVEhFUlNFUklFU1ZQOU1QNEjj3JWbBg==";

    // Define Fairplay License Details (Details for demonstration only)
    private static String FAIRPLAY_KEY = "344A9430A36BB073B1E97A4FF185900E";
    private static String FAIRPLAY_IV = "b4839b42647b884eb5a0804d3699aac4";
    ...
    public void testDrmEncoding() throws IOException, BitmovinApiException, UnirestException, URISyntaxException, RestException, InterruptedException
    {
    	...
    	// Build Basic Encoding - Name will include a backend identifier (Content ID etc)
        Encoding encoding = new Encoding();
        encoding.setName("Encoding JAVA with DRM");
        encoding.setCloudRegion(cloudRegion);
        
        // For Premium Content - Use Kubernetes with on-premise infrastructure
        //encoding.setInfrastructureId(infrastructureId);
        
        // Create the Encoding
        encoding = bitmovinApi.encoding.create(encoding);

        // Create HTTP Input Source (Non-Premium)
        HttpInput input = new HttpInput();
        input.setHost(HTTP_INPUT_HOST);
        input = bitmovinApi.input.http.create(input);
        
        // Create Local Input Source (Premium)
        //LocalInput input = new LocalInput();
        //input.setPath(LOCAL_INPUT_ROOT);
        //input = bitmovinApi.input.local.create(input);

        // Optimally - As all content is pulling from the same place, we can make 1 input, and re-use by calling it's ID
        //HttpInput input = bitmovinApi.input.http.get("<ID_OF_HTTP_INPUT>";
        //LocalInput input = bitmovinApi.input.local.get("<ID_OF_LOCAL_INPUT>");
        
        // Setup S3 Output - Assuming this is the first run
        ...
        // Optimally - As all content is pushing to S3, we can make 1 output, and re-use by calling it's ID
        //S3Output output = bitmovinApi.output.s3.get("<ID_OF_S3_OUTPUT>");
      
        // Define Audio Profile
        ...
        // Select Input Path to Source File (Change to Local Input Path if using On-Premise)
        InputStream inputStreamAudio = new InputStream();
        inputStreamAudio.setInputPath(HTTP_INPUT_PATH);
        //inputStreamAudio.setInputPath(LOCAL_INPUT_PATH);
        ...
        inputStreamVideo.setInputPath(HTTP_INPUT_PATH);
        //inputStreamVideo.setInputPath(LOCAL_INPUT_PATH);
        ...
        // Add Audio Source to Audio Encoding Configuration
        ...
        // Assign Video Source to all Video Encoding Profiles
        ...
        // Use ID of created S3 Output to assign output to encoding session
        EncodingOutput encodingOutput = new EncodingOutput();
        encodingOutput.setOutputId(output.getId());
        encodingOutput.setOutputPath(OUTPUT_BASE_PATH);

        // Define profiles to be muxed into fMP4 segments and segment into 4s segments
        ...
        // Build PlayReady DRM Settings using variables defined
        ...
        // Define Final fMP4 file location and setup output
        ...
        // Apply Playready to Base fMP4 Muxing created lines 237-242
        ...
        // Build Widevine DRM Settings using variables defined
        ...
        // Apply Widevine to Base fMP4 Muxing created lines 237-242
        ...
        // Build FairPlay DRM Settings using variables defined
        ...
        // Define Final TS file location and setup output
        ...
        // Apply FairPlay DRM to TS Muxing
        ...
        // Monitor Encoding Status. Wait for Finished Status before building manifests
        ...
        // Build DASH Manifest - Single DASH Manifest contains PlayReady & Widevine fMP4 files
        ...
        // Build HLS Manifest - HLS Manifest contains FairPlay TS files
